...
>
> **Compatibility boundary:** Generation 2 (`application_id=0x4E425249`, `user_version=2`), targeting Linux/amd64. Complete beta.4 and eleven exact earlier manifests support atomic upgrades to 110 tables, preserving existing data and configuration. The two new games start disabled. Alpha/Generation 1 still requires an explicit fresh cutover. See the [deployment guide](docs/deployment.md#database-compatibility-and-version-changes).
>
...
- Account export schema 7 includes both wallets, asset-tagged entries, independent check-ins, game payment sources and lifetime newcomer completions while excluding secrets, other users and internal capacity, scheduling and audit data.
...
Bidding Duel offers 13 simultaneous hidden-card rounds. Likes Battle combines five characters, eight harnesses and full skill/buff rules with a shared battery, synchronized five-second settlement and dynamic resource feedback. Uncapped API reserve and gold use numeric counters. Character and harness illustrations currently reuse bundled placeholders. Both games support original-asset refunds, safe 30-day player history and administrator-only anonymous archives and resumable exports. See [the bilingual game guide](docs/duel-games.md).
