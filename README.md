...
- Administrators and stewards can fetch model lists for a single available donated key, every available key in one donation, or all available donated keys. Bulk operations include other pages and ignore list filters, show progress and support stopping further requests. Failed fetches preserve the existing catalog; manual entries and model bindings remain unchanged.
