...
>
> **Current source:** development toward `1.0.0-rc.3`; it has not been tagged or deployed by this change. The published rc.2 tag remains unchanged. The supported upgrade source for this newer code is the rc.2 maintenance commit `db959c64674afc531046a63066de0464725d439c`, preserving existing data, configuration and instance legal text. Generation 2 (`application_id=0x4E425249`, `user_version=2`) and Linux/amd64 remain the target. Unreleased intermediate schemas are outside this guarantee; Alpha/Generation 1 requires a fresh cutover. See the [deployment guide](docs/deployment.md#database-compatibility-and-version-changes).
>
> Source repository: [github.com/waiting-here/NonbiriAPI](https://github.com/waiting-here/NonbiriAPI)

## Highlights

- Optional game-credit loans show principal, fee, game credits received, interest and general credits deducted. New rolling seven-day net-profit boards cover all games, Fishing and Blackjack alongside the existing spending and profit boards; losses offset gains, and charity visibility keeps its own opt-in setting.
...
- Account export schema 10 retains the earlier safe account and game data and adds activity wallets, exchange receipts, safe image-task outcomes and inactivity actions. Raw errors, source facts, risk evidence, prompts, images, private configuration and other users' identities remain excluded.
...
The newer source keeps Generation 2 and supports an upgrade from the complete rc.2 maintenance database at `db959c6`. Existing accounts, balances, donations, model bindings, games, settings and custom legal text are preserved; old full stewards migrate to level 6 and new activity assets start separately. Unknown development schemas are unsupported. A downgrade requires the complete matching stopped snapshot. Fresh databases keep maintenance enabled and public feature gates closed. Alpha/Generation 1 requires a fresh cutover.
