...
func New(options Options) (*Service, error) {
	...
	ownsLimiter := options.Limiter == nil
...
func (service *Service) Close() error {
	...
	if service.limiter != nil && service.ownsLimiter {
...
func (service *Service) readSnapshot(ctx context.Context, tx *sql.Tx) (fishingconfig.Snapshot, int64, error) {
	rows, err := tx.QueryContext(ctx, `SELECT key,value FROM site_config WHERE key IN (`+placeholders(len(snapshotKeys()))+`)`, stringArgs(snapshotKeys())...)
	if err != nil {
		return fishingconfig.Snapshot{}, 0, classifyDB(err)
	}
	values := make(map[string]string, len(snapshotKeys()))
	for rows.Next() {
		var key string
		...
		if !value.Valid {
			rows.Close()
			return fishingconfig.Snapshot{}, 0, ErrInvariant
		}
	...
	if len(values) != len(snapshotKeys()) {
		return fishingconfig.Snapshot{}, 0, ErrInvariant
	}
	var revision int64
	...
	snapshot, err := fishingconfig.CompileConfig(values)
	if err != nil {
		return fishingconfig.Snapshot{}, 0, ErrInvariant
	}
