...
func TestStartFishingRequiresLiveRuntimeCapability(t *testing.T) {
	...
	fixture.service.capability = func() bool { return false }
	tx, err := fixture.database.BeginTx(context.Background(), nil)
	if err != nil {
		t.Fatal(err)
	}
	defer tx.Rollback()
	snapshot, _, err := fixture.service.readSnapshot(context.Background(), tx)
	if err != nil || !snapshot.FishingEnabled || fixture.service.Module().Available("", "") {
