...
func TestFinancialPortsRejectForeignSourcesBeforeLedgerMutation(t *testing.T) {
	...
	for _, ddl := range []string{
		`CREATE TABLE game_fishing_batches(id TEXT,user_id INTEGER,entry_total_milli INTEGER,payout_total_milli INTEGER,state TEXT,operation_id TEXT)`,
		`CREATE TABLE game_linklink_sessions(id TEXT,user_id INTEGER,price_milli INTEGER,operation_id TEXT,state TEXT)`,
		`CREATE TABLE game_rps_queue(id TEXT,user_id INTEGER,account_id INTEGER,reserved BLOB,reservation_operation_id TEXT)`,
		`CREATE TABLE game_rps_sessions(id TEXT,account_id INTEGER,state TEXT,terminal_operation_id TEXT,player_pool BLOB)`,
		`CREATE TABLE game_rps_seats(session_id TEXT,user_id INTEGER,deletion_state TEXT)`,
	...
	for _, query := range []struct {
		...
		{`INSERT INTO game_fishing_batches VALUES(?,1,1000,1200,'reserved',?)`, []any{id("fb_"), operation}},
		{`INSERT INTO game_linklink_sessions VALUES(?,1,1000,?,'active')`, []any{id("ll_"), operation}},
		{`INSERT INTO game_rps_queue VALUES(?,1,10,?,?)`, []any{id("rpsq_"), db.EncodeU128(amount), operation}},
		{`INSERT INTO game_rps_sessions VALUES(?,11,'started',?,?)`, []any{id("rps_"), operation, db.EncodeU128(amount)}},
		{`INSERT INTO game_rps_seats VALUES(?,1,'active')`, []any{id("rps_")}},
	...
	for _, module := range []struct{ id, prefix string }{{"fishing", "fb_"}, {"linklink", "ll_"}, {"rps", "rpsq_"}} {
		for _, scenario := range []string{"foreign owner", "foreign actor", "wrong prefix", "wrong amount", "negative amount", "invalid operation"} {
