...
func TestReadClosedSourcesOwnershipStableOrderAndNoSideEffects(t *testing.T) {
	...
	summary, err := service.HomeSummary(context.Background(), 1)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(summary.Continue) != 3 || summary.Continue[0] != (game.ContinueItem{Game: game.FishingID, ResourceID: fishingContinue, State: "settlement_pending", RouteID: "game-fishing"}) ||
		summary.Continue[1] != (game.ContinueItem{Game: game.LinkLinkID, ResourceID: linkID, State: "active", RouteID: "game-linklink"}) ||
		summary.Continue[2] != (game.ContinueItem{Game: game.RPSID, ResourceID: rpsID, State: rps.StateStarted, RouteID: "game-rps"}) {
	...
	foreign, err := service.HomeSummary(context.Background(), 2)
...
func TestReadMergesPendingFIFOAndRejectsImpossibleRPSUnion(t *testing.T) {
	...
	if _, err := service.HomeSummary(context.Background(), 1); !errors.Is(err, host.ErrInvariant) {
...
func TestHTTPStrictClosedWireAuthorizationAndMaintenanceRegistrar(t *testing.T) {
	...
	service := newHomeService(t, database, authorizer, links, rpsSource)
	routes := &homeRoutes{}
	if err := service.RegisterRoutes(host.Registrars{User: routes, Admin: routes, Continuation: routes, Maintenance: routes}); err != nil {
		t.Fatal(err)
	}
	if routes.method != http.MethodGet || routes.path != host.RouteHomeSummary || routes.handler == nil {
		t.Fatalf("registered=%s %s handler=%v", routes.method, routes.path, routes.handler != nil)
	}

	valid := routes.request(http.MethodGet, host.RouteHomeSummary, "", 1)
	...
	for _, request := range []struct{ target, body string }{{host.RouteHomeSummary + "?extra=1", ""}, {host.RouteHomeSummary + "?", ""}, {host.RouteHomeSummary, `{}`}} {
	...
	unauthorized := routes.request(http.MethodGet, host.RouteHomeSummary, "", 1)
	...
	maintenance.handler = routes.handler
	before := authorizer.calls
	blocked := maintenance.request(http.MethodGet, host.RouteHomeSummary, "", 1)
	...
	rpsSource.err = host.ErrResourceLimit
	limited := routes.request(http.MethodGet, host.RouteHomeSummary, "", 1)
...
func (routes *homeRoutes) RegisterAdminRoute(string, string, http.Handler) error { return nil }
func (routes *homeRoutes) RegisterContinuationUserRoute(string, string, resources.AuthenticatedContinuationHandler) error {
	return nil
}
func (routes *homeRoutes) Register(maintenance.ContinuationKind, maintenance.ContinuationRegistration) error {
	return nil
}

func newHomeService(t *testing.T, database *sql.DB, authorizer *homeAuthorizer, links *homeLinkSource, rpsSource *homeRPSSource) *host.Service {
	t.Helper()
	production, err := builtinconfig.Registry()
	if err != nil {
		t.Fatal(err)
	}
	registry := game.NewRegistry()
	factories := map[string]host.Factory{}
	projections := map[string]func(context.Context, *sql.Tx, int64) (game.HomeSummary, error){
		game.FishingID: func(ctx context.Context, tx *sql.Tx, userID int64) (game.HomeSummary, error) {
			return new(fishingruntime.Service).ModuleHomeSummaryTx(ctx, tx, userID)
		},
		game.LinkLinkID: func(ctx context.Context, tx *sql.Tx, userID int64) (game.HomeSummary, error) {
			value, err := links.HomeSummaryTx(ctx, tx, linklink.HomeSummaryInput{UserID: userID})
			if err != nil {
				return game.HomeSummary{}, err
			}
			return linklink.ProjectHomeSummary(value)
		},
		game.RPSID: func(ctx context.Context, tx *sql.Tx, userID int64) (game.HomeSummary, error) {
			value, err := rpsSource.HomeSummaryTx(ctx, tx, userID)
			if err != nil {
				return game.HomeSummary{}, err
			}
			return rps.ProjectHomeSummary(value)
		},
	}
	for _, descriptor := range production.Descriptors() {
		...
		descriptor.ContinuationIDs = nil
		if err := registry.Register(descriptor); err != nil {
			t.Fatal(err)
		}
		project := projections[descriptor.ID]
		factories[descriptor.ID] = func(host.Services) (*host.Module, error) {
			return &host.Module{
				ValidatePersistedState: func(context.Context) error { return nil }, RecoverBeforeListen: func(context.Context, int64, int, time.Time) (host.WorkResult, error) { return host.WorkResult{}, nil },
				RegisterRoutes: func(host.Registrars) error { return nil }, StartWorker: func(context.Context) error { return nil }, Close: func() error { return nil }, Available: func(string, string) bool { return true }, ReadyTx: func(context.Context, *sql.Tx) bool { return true },
				UserSnapshotTx: func(context.Context, *sql.Tx, int64, int64, game.ConfigValue) (game.UserSnapshot, error) {
					return game.UserSnapshot{}, nil
				},
				HomeSummaryTx: project, ActiveCountsTx: func(context.Context, *sql.Tx) (game.ActiveCounts, error) { return game.ActiveCounts{}, nil },
				ExportTx: func(context.Context, *sql.Tx, int64, int64, int) (any, host.Finalizer, error) { return nil, nil, nil }, PrepareDeleteTx: func(context.Context, *sql.Tx, int64, int64) (host.Finalizer, error) { return nil, nil }, Retain: func(context.Context, int64, int, time.Time) (host.WorkResult, error) { return host.WorkResult{}, nil },
			}, nil
		}
	}
	...
	service, err := host.New(host.Options{Database: database, UserAuthorizer: authorizer, AdminAuthorizer: host.AdminAuthorizerFunc(func(context.Context, *sql.Tx) error { return nil }), Registry: registry, Factories: factories})
	if err != nil {
		t.Fatal(err)
	}
