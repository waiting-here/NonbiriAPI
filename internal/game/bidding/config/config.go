...
const (
	ID            = "bidding"
	Version       = 1
	EnabledKey    = "game_bidding_enabled"
	QueueSeconds  = 120
	QueueCapacity = 4096
)

var modeKeys = [...]string{"tier1", "tier2", "tier3"}
var defaultTickets = [...]int64{5_000_000, 10_000_000, 50_000_000}
...
func ModeEnabledKey(mode string) string { return "game_bidding_" + mode + "_enabled" }
func TicketKey(mode string) string      { return "game_bidding_" + mode + "_ticket_milli" }
func RakeKey(mode, destination string) string {
	return "game_bidding_" + mode + "_rake_" + destination + "_bp"
}
...
func (value compiled) UserWire(available func(mode, spec string) bool) json.RawMessage {
	...
	}{Enabled: value.Enabled(), Modes: make(map[string]userMode), QueueSeconds: QueueSeconds, JokerSeconds: 10, BidSeconds: 20, QueueCapacity: QueueCapacity}
