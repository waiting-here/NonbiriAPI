...
func (s *Store) ListEndpoints(ctx context.Context, userID int64) ([]Endpoint, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, user_id, connector_type, base_url, note, enabled, created_at, updated_at FROM endpoints WHERE user_id=? ORDER BY id`, userID)
	if err != nil {
...
func (s *Store) GetEndpoint(ctx context.Context, userID, id int64) (Endpoint, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT id, user_id, connector_type, base_url, note, enabled, created_at, updated_at FROM endpoints WHERE id=? AND user_id=?`, id, userID)
...
func scanEndpointRow(row *sql.Row) (Endpoint, error) {
	var ep Endpoint
	var enabledInt int
	err := row.Scan(&ep.ID, &ep.UserID, &ep.ConnectorType, &ep.BaseURL, &ep.Note, &enabledInt, &ep.CreatedAt, &ep.UpdatedAt)
	if err != nil {
		return Endpoint{}, err
	}
...
func scanEndpoints(rows *sql.Rows) ([]Endpoint, error) {
	...
	for rows.Next() {
		var ep Endpoint
		var enabledInt int
		if err := rows.Scan(&ep.ID, &ep.UserID, &ep.ConnectorType, &ep.BaseURL, &ep.Note, &enabledInt, &ep.CreatedAt, &ep.UpdatedAt); err != nil {
