...
func TestGenerationTwoConfigDefaultsUseAuthoritativeSubsystemValues(t *testing.T) {
	...
	wantFishing := map[string]string{
		...
		fishingconfig.FishingStandardRTPKey:              "100",
		fishingconfig.FishingPremiumRTPKey:               "100",
...
func TestGenerationTwoConfigScalarBoundaries(t *testing.T) {
	valid := []struct{ key, value string }{
		...
		{"charity_donation_notice_zh", "第一段\n\t第二段"},
		{"charity_donation_notice_en", strings.Repeat("x", generationTwoMaxDonationNoticeBytes)},
	...
	invalid := []struct{ key, value string }{
		...
		{"charity_donation_notice_en", strings.Repeat("x", generationTwoMaxDonationNoticeBytes+1)},
		{"charity_donation_notice_zh", "bad\x00value"},
