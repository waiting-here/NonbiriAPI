...
SELECT 1 FROM report_targets WHERE case_id=? AND owner_user_id=?)`, caseID, snapshot.ownerUserID).Scan(&ownerAlreadySeen); err != nil {
 ...
 id,case_id,target_seq,endpoint_key_id,key_ref,owner_user_id,owner_discord_id,owner_display_name,
...
VALUES(?,?,?,?,?,?,?,?,?,?,?,?,'protected',?,NULL,?,?)`,
		targetID, caseID, newSequence, snapshot.keyID, keyRef[:], snapshot.ownerUserID, ownerDiscord, snapshot.displayName,
...
SELECT 1 FROM report_targets WHERE case_id=? AND owner_user_id=?)`, caseID, snapshot.ownerUserID).Scan(&ownerAlreadySeen); err != nil {
 ...
 connector_type,canonical_base_url,key_display_head,key_display_tail,state,discovered_version,decided_version,created_at,updated_at)
VALUES(?,?,?,?,?,?,?,?,?,?,?,?,'released',?,?,?,?)`,
		targetID, caseID, newSequence, snapshot.keyID, keyRef[:], snapshot.ownerUserID, ownerDiscord,
