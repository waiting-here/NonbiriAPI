...
func TestRegisterRoutesAndAccountLifecycleHTTP(t *testing.T) {
	...
	if exportResponse.Code != http.StatusOK ||
		exportResponse.Header().Get("Content-Disposition") != `attachment; filename="nonbiriapi-account-export-v6.json"` ||
	...
	if err := json.Unmarshal(exportResponse.Body.Bytes(), &exported); err != nil || exported["schema_version"] != float64(6) {
