...
> Status: **active** — current table describes the `v1.0.0-alpha.3` fresh-only schema. Future rows must be added in the same change that introduces them; planning text alone is not current coverage.
>
...
## Current coverage (`v1.0.0-alpha.3`)

| Table / column | User link | Export | Delete | Retention | Privacy |
|---|---|---|---|---|---|
| `users` (identity, lang, limits, `game_profile_public`) | self | yes (whitelisted raw/effective limits and the game-profile visibility flag; export package `schema_version` 3) | row removed by `DeleteUserAccount` | account lifetime | no Discord token/credential stored; profile text bounded; the game leaderboard omits every identity field unless the flag is true and always projects current profile data rather than retaining a leaderboard snapshot |
...
| `users` (level columns: `level`, `auto_level`) | self | yes (small integer state values `manual_level` nullable / `auto_level` in the export user section; `effective_level`/`manual_level` also projected in `/api/session` and `/api/me`) | removed with the row | account lifetime; `auto_level` is a persisted only-ever-raised high-water mark (lazy CAS promotion on authoritative resolution; no downgrade path in alpha.3), `level` is the nullable administrator-set manual override | server-set capability state; capability checks (banner ≥2, check-in cap bypass ≥3, steward ≥5) are decided server-side per use from the authoritative resolver — the projected level is display/state data, never authorization input; the administrator row is excluded from the level system entirely; no violation-window or threshold-configuration data exported |
| `credit_ledger` | `user_id` FK CASCADE; `actor_user_id` FK SET NULL | yes (the owner's full bounded ledger: id, kind, deltas and resulting balances as canonical decimal strings, nullable opaque actor id, bounded reason, settlement correlation, created_at) | cascade on user delete (no late-write path exists: every write goes through the same-transaction balance update on a live user row) | account lifetime; append-only, never rewritten or cleaned early | kind/delta/balance/reason only; game rows use `game_reserve` / `game_settlement` / `game_release`, always keep `donation_credit_delta=0`, and correlate by opaque settlement id; `anti_abuse_penalty` uses this existing ledger; the actor is an opaque nullable id (never a username); no secret, ciphertext, or request content exists on ledger rows |
| `checkins` | `user_id` FK CASCADE | yes (`checkins` section: the owner's check-in history — the site-local calendar date `YYYY-MM-DD` rendered under the frozen site offset and the granted award as a canonical decimal string; export package `schema_version` 3) | cascade on user delete (the day's site-activity rollup recompute covers the check-in contribution via the same-transaction activity row) | **account lifetime** — deliberately NOT part of any time-based retention sweep | dates + award only by construction: no streak, location, or device column exists anywhere in the table; the client can never supply the award or the day |
...
| `request_logs` | `user_id` FK CASCADE; `endpoint_key_id` FK SET NULL | yes (bounded metadata summary; **no content**) | cascade on user delete; late `RecordRequest` uses `INSERT ... SELECT ... WHERE EXISTS users` (atomic no-op) | **30-day retention cleanup**; at-most-once by `attempt_id` partial unique index | metadata + bounded `error_diag` only; content never persisted. Token values use the neutral four-bucket form (`uncached_input_tokens`/`cache_write_input_tokens`/`cache_read_input_tokens`/`output_tokens`) with legacy prompt/completion/total mirrors; `endpoint_base_url` is the bounded dispatch-time canonical base-URL snapshot (owner-visible metadata, sanitized at the persistence boundary); `route_kind` distinguishes `personal`/`charity`, and the charity rail writes `charity_reservation_id` plus the three charge columns (`original_charge`/`user_charge`/`donor_reward`, neutral defaults on personal rows). The admin-station CSV/JSON export is metadata-only, unpaginated with fail-closed row/byte bounds and per-cell formula-injection sanitization; the user station has no export. The user projection JOINs current notes of the caller's own resources (empty after deletion); no admin/steward projection ever selects notes or the user-chosen model name. Donor resources stay unidentifiable from a charity consumer's log row: the write path stores NULL for `endpoint_key_id` unless the key belongs to the logging user, and both the consumer-facing user projection and the level-5 steward projection (`/api/steward/logs`) additionally blank `endpoint_base_url`/`upstream_model_id` on `route_kind='charity'` rows. Steward resource filters run against that blanked projection, so guessing a donor URL/model cannot select or count a charity row; only the administrator projection shows the dispatch-time base URL and upstream model that actually served a request |
| derived upstream `safety_identifier` (not persisted) | current consumer user id + dispatch-time canonical upstream origin | no; the platform API never returns it | no stored row exists; deletion removes the source user id and future derivation becomes impossible | request lifetime only; the derived subkey is process-memory-only | v3 HMAC pseudonym sent only to the selected upstream for risk attribution; stable only for the same deployment key, user, and canonical origin (path excluded), and different across origins/deployments. It contains no raw account/Discord id but remains linkable within that scope, so the privacy template describes it as pseudonymous rather than anonymous. It is never logged or included in exports |
...
| `user_activity_daily` | `user_id` FK CASCADE | yes (`activity_daily` section: the user's own per-day summary — day key, product_active, four-bucket token counters, checkins, console_writes, `game_active` and `game_rounds`; export package `schema_version` 3) | cascade on user delete; account deletion recomputes every affected day's site rollup from the surviving rows in the same transaction | **400-day retention cleanup** (`CleanupActivityBefore`, startup + every 6h via `runMaintenanceSweep`) | counters and site-local day keys only — no model names, no request content; day keys never generated while the site timezone offset is unset |
...
| `game_settlements` / `game_rounds` | both belong to one `user_id`; the round has a composite FK to its settlement | yes (bounded schema-v3 sections include state, entry/payout, event sequence and lifecycle timestamps; idempotency hashes, retry metadata and internal scheduling fields are excluded) | cascade on user delete; the delete boundary first blocks new user admission and cancels in-flight work, then the same account transaction removes settlement, round, outcome and correlated ledger rows; late settlement/ACK paths are conditional and become no-ops | pending rows are **never** age-removed; only terminal rounds are removed after 30 days measured from `settled_at`, in bounded batches; the paired settlement and outcome cascade with the round | server-authoritative ids and integer economic values only; the client idempotency key is retained solely as SHA-256 and is never exported; retry errors use a closed class, never upstream text; complete SQLite snapshots include these tables and their correlation indexes |
| `game_fishing_outcomes` | `round_id` FK CASCADE, transitively owned by the round user | yes (bounded schema-v3 outcome: bait, species/tier, size and derived display/economic values) | cascade through round/account deletion | same 30-day terminal-round lifetime; pending outcome follows its pending round and is never age-removed | strongly typed server-generated values only; no client text, secret or request content |
| `game_fishing_best` | `user_id` FK CASCADE; nullable `round_id` FK SET NULL | yes (bounded schema-v3 personal-best row) | cascade on account deletion; terminal-round retention only clears the optional round correlation | account lifetime; one row per user by primary key | server-generated species/tier/size/time only; public leaderboard identity is dynamically projected from the current user profile and never stored in this row; complete SQLite snapshots include the row |
