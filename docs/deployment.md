...
This guide describes the supported single-instance model: one Linux/amd64 binary, a dedicated service user, local SQLite and a TLS reverse proxy. The latest published source prerelease is 1.0.0-rc.2; newer source is in development toward rc.3 and is not a new tag or deployment. Its supported upgrade source is the complete rc.2 maintenance commit db959c64674afc531046a63066de0464725d439c. Alpha deployments require a fresh cutover. Verify compatibility, backups, configuration, instance legal text and changed behavior before opening a deployment. See [configuration.md](configuration.md) for environment settings and [image-activity.md](image-activity.md) for activity operations.
