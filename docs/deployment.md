...
Before a writable source open, existing files are copied through no-follow read-only handles to a private validation directory. Header, manifest, foreign keys, indexes, sidecars and contextual credential envelopes are validated there. Unsupported sources are rejected without repair or new source-side WAL/SHM files. The fifteen source classes include `preRouting`, `preKeyLimits`, `preResponseStarts`, `preBetaTwo`, `preBrowse`, `preQuotaCleanup`, `preStewardHoldRead`, `preModelTokenReserve`, `preHourlyQuota`, `preEmbedding`, `preBetaFour`, `preRCOne`, `preBlackjack`, `preRandomness` and `preGatewayPolicy`. These are exact manifests, not permission to accept arbitrary intermediate schemas.
