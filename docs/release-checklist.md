...
- [ ] Confirm the beta.2 candidate is source-first, the production target is Linux/amd64, and the validation binary was built from the exact candidate commit with `CGO_ENABLED=0 -tags dist -trimpath`. The unreleased candidate publishes no official binaries, container images, or installers.
- [ ] Verify the beta.2 candidate scope against implementation and evidence: browser-local time-point handling, bounded numbered pagination, source/model browsing, recurring donation quotas, model-level access and descriptions, announcements, and game presentation/audio. Do not describe an unmerged or unverified behavior as shipped.
...
- [ ] Test each of the four explicitly supported earlier Generation 2 manifests and the two exact deployed intermediate manifests through its additive update and a second normal startup; verify that accounts, resources, balances, settings, routing choices, key limits, and historical facts are preserved.
