...
- [ ] Round-trip all four legal overrides and both donation-guidance fields with multiline Chinese/English, tabs, LF/CRLF, multibyte text, and values near their respective 65,536-byte and 8,192-byte limits through save → GET → refresh → re-save. Verify anonymous legal pages and the user donation form at 390 px, keyboard-only, and screen-reader semantics.
