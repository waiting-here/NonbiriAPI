...
- [ ] Record the four exact deployed beta.2 intermediate structures: `preBrowse` (recurring-quota side table), `preQuotaCleanup` (browse indexes), `preStewardHoldRead` (cleanup indexes), and `preModelTokenReserve` (steward held-read audit and Fishing length tables); confirm the latest structure adds only the sparse model-level Token reserve override table.
...
- [ ] Confirm the beta.2 release is source-first, the production target is Linux/amd64, and the validation binary was built from the exact release commit with `CGO_ENABLED=0 -tags dist -trimpath`. The source release publishes no official binaries, container images, or installers.
- [ ] Verify the beta.2 release scope against implementation and evidence: browser-local time-point handling, bounded numbered pagination, source/model browsing, recurring donation quotas, model-level access and descriptions, per-model Token reserve overrides with global inheritance, announcements, and game presentation/audio. Do not describe an unmerged or unverified behavior as shipped.
...
- [ ] Round-trip all four legal overrides and both donation-guidance fields with multiline Chinese/English, tabs, LF/CRLF, multibyte text, and values near their respective 65,536-byte and 8,192-byte limits through save → GET → refresh → re-save. Verify anonymous legal pages and the user donation form at 390 px and with ordinary keyboard, mouse, and touch input.
...
- [ ] Test each of the four explicitly supported earlier Generation 2 manifests and the four exact deployed intermediate structures (`preBrowse`, `preQuotaCleanup`, `preStewardHoldRead`, and `preModelTokenReserve`) through its additive update and a second normal startup; verify that accounts, resources, balances, settings, routing choices, key limits, and historical facts are preserved.
- [ ] Verify model-level Token reserve configuration and source preservation: create omission/`null` inheritance, PATCH omission preservation and `null` clearing, per-request behavior, admitted-value freezing, deletion cleanup, owner-export exclusion, and that the latest source adds only the sparse override table while existing rows remain unchanged.
