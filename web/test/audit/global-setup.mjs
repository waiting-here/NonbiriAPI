...
export default async function setup() {
  const root = resolve('..');
  const statePath = process.env.NONBIRI_AUDIT_BROWSER_STATE;
  if (!statePath) throw new Error('The audit state path is missing.');
  ...
  const child = spawn(
    binary,
    ['-test.run=^TestAuditBrowserFixture$', '-test.v', '-test.timeout=9m'],
    {
      cwd: root,
      env: { ...process.env, NONBIRI_AUDIT_BROWSER_STATE: statePath },
      windowsHide: true,
      stdio: ['ignore', 'pipe', 'pipe'],
    },
  );
