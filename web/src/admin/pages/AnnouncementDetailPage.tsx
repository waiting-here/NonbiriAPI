...
export function AnnouncementDetailPage() {
  ...
  const preview = useMutation({
    ...
    mutationFn: (input: DraftState) =>
      previewAnnouncement(announcementId, {
  ...
  const lifecycle = useRetainedOperation(
    async (
      ...
      if (input.action === 'publish')
        return publishAnnouncement(announcementId, input.revision, key);
      if (input.action === 'withdraw')
        return withdrawAnnouncement(announcementId, input.revision, input.reason, key);
    ...
    <div className="page ops-page">
      ...
      <Card>
        <div className="ops-toolbar">
          ...
          <span>
            {t('admin.announcements.detail.authorityRevision', { revision: item.revision })}
          </span>
          {item.published ? (
            <span>
              {t('admin.announcements.detail.publishedRevision', {
      ...
      <Card>
        ...
        <div className="ops-actions">
          ...
          >
            {t('admin.announcements.detail.preview')}
          </button>
          <button
          ...
          >
            {t('admin.announcements.detail.discard')}
          </button>
        </div>
      </Card>
      {preview.error ? (
        ...
        <Card>
          <h2>
            {t('admin.announcements.detail.serverPreview', {
          ...
          <div className="ops-grid">
            {preview.data.rendered_zh ? (
              <section>
                <h3>{t('admin.announcements.detail.chinese')}</h3>
                <SafeAnnouncementBody html={preview.data.rendered_zh} />
              </section>
            ) : null}
            {preview.data.rendered_en ? (
              <section>
                <h3>{t('admin.announcements.detail.english')}</h3>
      ...
      {item.published ? (
        <Card>
          <h2>{t('admin.announcements.detail.currentProjection')}</h2>
          <p>
            {t('admin.announcements.detail.publishedHint', {
      ...
      <Card className="ops-danger">
        <h2>{t('admin.announcements.detail.lifecycleTitle')}</h2>
        {lifecycle.error ? <ErrorState error={lifecycle.error} /> : null}
        <label className="ops-form-field">
          <span>{t('admin.announcements.detail.reason')}</span>
          <input
            value={reason}
            maxLength={1024}
        ...
        <div className="ops-actions">
          ...
          >
            {item.state === 'published'
              ? t('admin.announcements.detail.publishUpdate')
              : item.published
                ? t('admin.announcements.detail.republish')
                : t('admin.announcements.detail.publish')}
          </button>
          {item.state === 'published' ? (
            <button
            ...
            >
              {t('admin.announcements.detail.withdraw')}
            </button>
          ) : null}
          <button
            className="btn btn-danger"
          ...
          >
            {t('admin.announcements.detail.permanentlyDelete')}
          </button>
        </div>
      </Card>
      {confirmation ? (
        <ConfirmDialog
          ...
          title={
            confirmation === 'delete'
              ? t('admin.announcements.detail.confirmDeleteTitle')
              : confirmation === 'withdraw'
                ? t('admin.announcements.detail.confirmWithdrawTitle')
                : item.state === 'published'
                  ? t('admin.announcements.detail.confirmUpdateTitle')
                  : t('admin.announcements.detail.confirmPublishTitle')
          }
          description={
            confirmation === 'delete'
              ? t('admin.announcements.detail.confirmDeleteBody')
              : confirmation === 'withdraw'
                ? t('admin.announcements.detail.confirmWithdrawBody')
                : t('admin.announcements.detail.confirmPublishBody')
          }
          confirmLabel={
            confirmation === 'delete'
              ? t('admin.announcements.detail.confirmDelete')
              : confirmation === 'withdraw'
                ? t('admin.announcements.detail.withdraw')
                : t('admin.announcements.detail.publish')
          }
