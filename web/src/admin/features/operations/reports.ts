...
export function normalizeReportBadge(value: unknown): ReportBadge {
  ...
  const status = record(
    root.by_status,
    ['pending_indexing', 'pending_review', 'approved_processing'],
    'report badge statuses',
  );
  const result = {
  ...
  if (
    BigInt(result.total) !==
    BigInt(result.by_status.pending_indexing) +
      BigInt(result.by_status.pending_review) +
      BigInt(result.by_status.approved_processing)
  )
    invalidResponse('report badge total');
  return result;
}
...
export interface ReportCaseSummary {
  ...
  retry: {
    ...
    last_error_class: 'db_busy' | 'internal_retryable' | 'invariant_violation';
  } | null;
  created_at: number;
...
export function normalizeReportSummary(value: unknown): ReportCaseSummary {
  ...
  if (
    (status === 'pending_indexing' || status === 'approved_processing') &&
    progress !== 'in_progress'
  )
    invalidResponse('report progress state');
  if (
  ...
  const counts = record(
    root.counts,
    ['materials', 'targets', 'distinct_owners', 'processed', 'deleted', 'released'],
    'report counts',
  );
  const normalizedCounts = {
    materials: decimal(counts.materials, 'report material count'),
    targets: decimal(counts.targets, 'report target count'),
    distinct_owners: decimal(counts.distinct_owners, 'report owner count'),
    processed: decimal(counts.processed, 'report processed count'),
    deleted: decimal(counts.deleted, 'report deleted count'),
    released: decimal(counts.released, 'report released count'),
  };
  if (
    BigInt(normalizedCounts.processed) > BigInt(normalizedCounts.targets) ||
    BigInt(normalizedCounts.deleted) > BigInt(normalizedCounts.targets) ||
    BigInt(normalizedCounts.released) > BigInt(normalizedCounts.targets)
  )
    invalidResponse('report target counts');
  ...
  if (root.retry !== null) {
    ...
    retry = {
      ...
      last_error_class: oneOf(
        item.last_error_class,
        ['db_busy', 'internal_retryable', 'invariant_violation'] as const,
        'report retry class',
      ),
    };
  }
  ...
  return {
    id: opaqueID(root.id, 'rpc_', 'report case id'),
    status,
    ...
    retry,
    created_at: unixSecond(root.created_at, 'report creation time'),
...
export interface ReportMaterial {
  id: string;
  note_text: string;
  reporter: { user_id: string; discord_id: string } | null;
  source_ip: string;
  created_at: number;
}
export function normalizeReportMaterial(value: unknown): ReportMaterial {
  const root = record(
    value,
    ['id', 'note_text', 'reporter', 'source_ip', 'created_at'],
    'report material',
  );
...
export interface ReportCaseDetail extends ReportCaseSummary {
  ...
  decision: {
    action: 'approve' | 'reject' | 'expire' | 'resume_processing';
    reason: string;
    actor_user_id: string | null;
    created_at: number;
  } | null;
}
export type ReportDecision = NonNullable<ReportCaseDetail['decision']>;
export function normalizeReportDecision(value: unknown): ReportDecision {
  const item = record(
    value,
...
export function normalizeReportDetail(value: unknown): ReportCaseDetail {
  ...
  const summary = normalizeReportSummary(
    Object.fromEntries(
      Object.entries(root).filter(([key]) => key !== 'materials' && key !== 'decision'),
    ),
  );
  let decision: ReportCaseDetail['decision'] = null;
  if (root.decision !== null) {
    decision = normalizeReportDecision(root.decision);
  }
  return {
    ...summary,
    materials: page(root.materials, 'report material page', normalizeReportMaterial),
    decision,
  };
}
...
export interface ReportTarget {
  ...
  target_seq: string;
  state:
    'protected' | 'deleted_by_owner' | 'deleted_by_account' | 'deleted_by_approval' | 'released';
  ...
  decided_version: string | null;
  donation_match_count: string;
  created_at: number;
  updated_at: number;
}
export function normalizeReportTarget(value: unknown): ReportTarget {
  ...
  const decided =
    root.decided_version === null
      ? null
      : decimal(root.decided_version, 'target decision version', { positive: true });
  if ((state === 'protected') !== (decided === null))
    invalidResponse('report target decision state');
  return {
    id: opaqueID(root.id, 'rpt_', 'report target id'),
    target_seq: decimal(root.target_seq, 'report target sequence', { positive: true }),
    state,
    endpoint_key_id:
      root.endpoint_key_id === null
        ? null
        : decimalID(root.endpoint_key_id, 'target endpoint key id'),
    key_ref: keyRef,
    owner,
    endpoint: {
      connector_type: oneOf(
    ...
    decided_version: decided,
    donation_match_count: decimal(root.donation_match_count, 'target donation match count'),
    created_at: unixSecond(root.created_at, 'target creation time'),
    updated_at: unixSecond(root.updated_at, 'target update time'),
  };
}

export type ReportDonationStatus = 'pending' | 'approved' | 'rejected' | 'deleted' | 'expired';
export type ReportDonationKeyState =
...
export const adminReportKeys = {
  ...
  list: (status: string, cursor: string | null) =>
    ['admin', 'operations', 'reports', 'list', status, cursor] as const,
  detail: (id: string, cursor: string | null) =>
    ['admin', 'operations', 'reports', 'detail', id, cursor] as const,
  targets: (id: string, cursor: string | null) =>
    ['admin', 'operations', 'reports', 'targets', id, cursor] as const,
  targetDonations: (id: string, targetId: string, cursor: string | null) =>
    ['admin', 'operations', 'reports', 'target-donations', id, targetId, cursor] as const,
};
export const getReportBadge = (signal?: AbortSignal) =>
  decoded('/admin/api/reports/badge', normalizeReportBadge, { signal });
export const getReports = (status: string, cursor: string | null) =>
  decoded(
    queryPath('/admin/api/reports', { status: status || undefined, cursor, limit: 50 }),
    (value) => page(value, 'report case page', normalizeReportSummary),
  );
export const getReportDetail = (id: string, cursor: string | null) =>
  decoded(
    queryPath(`/admin/api/reports/${encodeURIComponent(opaqueID(id, 'rpc_', 'report case id'))}`, {
      materials_cursor: cursor,
      materials_limit: 50,
    }),
    normalizeReportDetail,
  );
export const getReportTargets = (id: string, cursor: string | null) =>
  decoded(
    queryPath(
      `/admin/api/reports/${encodeURIComponent(opaqueID(id, 'rpc_', 'report case id'))}/targets`,
      { cursor, limit: 50 },
    ),
    (value) => page(value, 'report target page', normalizeReportTarget),
  );
export const getReportTargetDonations = (
  id: string,
  targetId: string,
...
export function normalizeReportDecisionReceipt(
  ...
  const root = record(
    value,
    ['id', 'status', 'material_version', 'target_version'],
    'report decision receipt',
  );
  const status = oneOf(
    root.status,
    ['approved_processing', 'rejected'] as const,
    'report decision receipt status',
  );
  ...
  return {
    ...
    status,
    material_version: decimal(root.material_version, 'report decision material version', {
      positive: true,
    }),
    target_version: decimal(root.target_version, 'report decision target version', {
      positive: true,
    }),
  };
}
