...
export function StewardPage() {
  ...
  if (authority.error) return <div className="page ops-page"><PageHeader title={t('user.steward.title')} description={t('user.steward.description')} /><ErrorState error={authority.error} onRetry={() => void authority.refetch()} /></div>;
  if (!allowed) return <div className="page ops-page"><PageHeader title={t('user.steward.title')} description={t('user.steward.description')} /><Card><p className="field-error" role="alert">{t('user.steward.accessDenied')} {t('user.steward.sensitiveStateCleared')}</p></Card></div>;

  return (
    <div className="page ops-page">
      <PageHeader title={t('user.steward.title')} description={t('user.steward.operationsDescription')} />
      <div className="ops-tabs" role="tablist" aria-label={t('user.steward.sectionsLabel')}><button className={section === 'logs' ? 'btn btn-primary' : 'btn btn-secondary'} type="button" role="tab" aria-selected={section === 'logs'} onClick={() => setSection('logs')}>{t('user.steward.logsTab')}</button><button className={section === 'charity' ? 'btn btn-primary' : 'btn btn-secondary'} type="button" role="tab" aria-selected={section === 'charity'} onClick={() => setSection('charity')}>{t('user.steward.charityTab')}</button><button className={section === 'maintenance' ? 'btn btn-danger' : 'btn btn-secondary'} type="button" role="tab" aria-selected={section === 'maintenance'} onClick={() => setSection('maintenance')}>{t('user.steward.maintenanceTab')}</button></div>
