...
const Bidding = lazy(async () => ({ default: (await import('../../bidding/BiddingGame')).BiddingGame }));
...
function DuelPage({ game }: { readonly game: DuelGame }) {
  ...
  const refreshWallets = useCallback(() => { void refetch(); }, [refetch]);
  if (snapshot.isPending) return <LoadingState />;
  if (snapshot.error && !isMaintenance(snapshot.error))
  ...
  return <main className="game-page"><Suspense fallback={<LoadingState />}>
    {game === 'bidding' ? <Bidding {...context} /> : <Likes {...context} />}
  </Suspense></main>;
}
export function BiddingPage() { return <DuelPage game="bidding" />; }
export function LikesPage() { return <DuelPage game="likes" />; }
