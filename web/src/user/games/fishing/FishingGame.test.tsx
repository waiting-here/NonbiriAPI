...
function installFishingFixture(options: FishingFixtureOptions = {}) {
  ...
  const calls = { start: 0, settle: 0, ack: 0, ackResolved: 0, profile: 0, games: 0, state: 0, leaderboards: 0, idempotencyKeys: [] as string[] };
  const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
    ...
    if (method === 'GET' && url.pathname === '/api/games') {
      ...
      if (reconciliationStarted && configReconcileFailuresRemaining > 0) {
        ...
        return json({ error: { code: 'service_unavailable', message: 'Configuration revalidation failed.' } }, 503);
      }
      return json({ ...config, master_enabled: masterEnabled, credits, game_profile_public: profilePublic, games: [{ ...config.games[0], enabled: gameEnabled }] });
    }
    ...
    if (method === 'GET' && url.pathname === '/api/games/fishing/state') {
      ...
      if (stateFailuresRemaining > 0) {
        ...
        return json({ error: { code: 'service_unavailable', message: 'Fishing state read failed.' } }, 503);
      }
      const stateGate = options.stateGates?.[calls.state - 1]
    ...
    if (method === 'GET' && url.pathname === '/api/games/fishing/leaderboard') {
      ...
      if (url.searchParams.get('board') === 'total') {
        if (reconciliationStarted && totalReconcileFailuresRemaining > 0) {
          ...
          return json({ error: { code: 'service_unavailable', message: 'Total leaderboard revalidation failed.' } }, 503);
        }
      ...
      if (reconciliationStarted && singleReconcileFailuresRemaining > 0) {
        ...
        return json({ error: { code: 'service_unavailable', message: 'Single leaderboard revalidation failed.' } }, 503);
      }
      const mineIsOutsideTopTwenty = typeof options.leaderboardMe?.rank === 'number' && options.leaderboardMe.rank > 20;
      const singleEntries = mineIsOutsideTopTwenty
        ...
        : [
          ...
          { rank: 2, species_key: 'koi', size_cm: 180, display_name: 'Public angler', avatar_url: 'https://cdn.discordapp.com/avatars/a/b.png?size=64', level4_badge: true, is_me: false },
        ];
      return json({
    ...
    if (method === 'POST' && url.pathname === '/api/games/fishing/rounds') {
      ...
      if (!options.startPreserveState) {
        state = { pending_round: { round_id: startRoundId, bait: 'worm', price: '2500000', created_at: 1, auto_settle_at: 2 }, unrevealed_result: previousUnrevealed, has_more_unrevealed: Boolean(previousUnrevealed) };
        credits = '2500000';
      }
      if (startFailuresRemaining > 0) {
        startFailuresRemaining -= 1;
        if (options.startFailureCreatesPending === false && !options.startPreserveState) state = previousState;
        return json({ error: { code: 'service_unavailable', message: 'Start response lost.' } }, 503);
      }
      return json({ round_id: startRoundId, game_id: 'fishing', game_version: 1, bait: 'worm', price: '2500000', credits, state: 'pending', created_at: 1, auto_settle_at: 2, idempotent_replay: false });
    }
    if (method === 'POST' && url.pathname.endsWith('/settle')) {
      ...
      if (settleFailuresRemaining > 0) {
        ...
        return json({ error: { code: 'service_unavailable', message: 'Temporary settlement outage.' } }, 503);
      }
      const settledResult = options.settleResults?.[settleResultIndex++] ?? options.settleResult ?? result;
      const previousUnrevealed = state.unrevealed_result ?? null;
      state = { pending_round: null, unrevealed_result: previousUnrevealed ?? settledResult, has_more_unrevealed: Boolean(previousUnrevealed) };
    ...
    if (method === 'POST' && url.pathname.endsWith('/ack')) {
      calls.ack += 1;
      if (options.ackDelayMs) await new Promise((resolve) => setTimeout(resolve, options.ackDelayMs));
      if (ackFailuresRemaining > 0) {
        ackFailuresRemaining -= 1;
        if (options.ackResponseLost) {
          state = options.ackFollowupState ?? { pending_round: null, unrevealed_result: null, has_more_unrevealed: false };
          calls.ackResolved += 1;
        }
        return json({ error: { code: 'service_unavailable', message: 'Temporary acknowledgement outage.' } }, 503);
      }
      state = options.ackFollowupState ?? { pending_round: null, unrevealed_result: null, has_more_unrevealed: false };
      calls.ackResolved += 1;
    ...
    if (method === 'PATCH' && url.pathname === '/api/me') {
      ...
      if (profileFailuresRemaining > 0) {
        ...
        return json({ error: { code: 'service_unavailable', message: 'Privacy update failed.' } }, 503);
      }
      const body = typeof init?.body === 'string' ? JSON.parse(init.body) as { game_profile_public?: unknown } : {};
      profilePublic = body.game_profile_public === true;
      if (options.profileResponseMode === 'no-content') return new Response(null, { status: 204, headers: { 'cache-control': 'no-store' } });
      if (options.profileResponseMode === 'malformed') return json({ user: {} });
      if (options.profileResponseMode === 'wrong') return json({ user: { game_profile_public: !profilePublic } });
  ...
  const media = {
    ...
    addEventListener(_type: 'change', listener: (event: MediaQueryListEvent) => void) { mediaListeners.add(listener); },
    removeEventListener(_type: 'change', listener: (event: MediaQueryListEvent) => void) { mediaListeners.delete(listener); },
    dispatchEvent: () => false,
  };
  ...
  return {
    ...
    setProfilePublic: (value: boolean) => { profilePublic = value; },
    setReconciliationStarted: () => { reconciliationStarted = true; },
...
describe('FishingGame', () => {
  it('renders a recovered server result, keeps anonymous identity omitted, and ACKs once', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('heading', { name: 'Catch confirmed' })).toBeInTheDocument(), { timeout: 5_000 });
    ...
    expect(Array.from(baitList.children).every((child) => child.getAttribute('role') === 'listitem')).toBe(true);
    expect(screen.getAllByRole('listitem')).toHaveLength(3);

    await rendered.user.click(screen.getByRole('checkbox', { name: 'Show my profile' }));
    await waitFor(() => expect(screen.getByRole('checkbox', { name: 'Show my profile' })).toBeChecked());
    await rendered.user.click(screen.getByRole('button', { name: 'Mark as viewed' }));
    await waitFor(() => expect(calls.ack).toBe(1));
    await waitFor(() => expect(screen.queryByRole('heading', { name: 'Catch confirmed' })).not.toBeInTheDocument());
    expect(calls.ack).toBe(1);
  });

  it.each([4, 5])('applies the L4 cosmetic theme for authoritative level %s and above', async (effectiveLevel) => {
  ...
  it('runs the local presentation through settle, then updates from server result and ACKs', async () => {
    ...
    expect(screen.getByRole('button', { name: /Premium lure.*Not enough available credits/ })).toBeDisabled();
    await rendered.user.click(cast);
    expect(calls.start).toBe(1);
    expect(calls.idempotencyKeys[0]).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i);
  ...
  it('restarts the active presentation when the reduced-motion preference changes', async () => {
    ...
    await waitFor(() => expect(document.querySelector('.fishing-stage')?.getAttribute('data-phase')).toBe('waiting'));
  ...
  it('keeps a paid pending round recoverable when settle fails, then retries explicitly', async () => {
    ...
    const retry = await screen.findByRole('button', { name: 'Retry settlement' }, { timeout: 7_000 });
    expect(calls.settle).toBe(1);
    expect(screen.getByText('Round pending')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Cast line' })).toBeDisabled();
    expect(screen.getByRole('button', { name: /Worm.*Finish the pending round first/ })).toBeDisabled();
  ...
  it('releases a failed-settlement retry marker once the server reports a terminal result', async () => {
    ...
    fixture.setState({ pending_round: null, unrevealed_result: result, has_more_unrevealed: false });
    await rendered.queryClient.refetchQueries({ queryKey: fishingKeys.state, exact: true });
    await waitFor(() => expect(screen.getByRole('heading', { name: 'Catch confirmed' })).toBeInTheDocument());
  ...
  it('supersedes an old settlement error when authority reports a newer pending round', async () => {
    ...
    const newerPending = { round_id: newerRoundId, bait: 'worm', price: '2500000', created_at: 1, auto_settle_at: 2 };
    const oldResult = { ...result, round_id: oldRoundId };
    const newerResult = { ...result, round_id: newerRoundId, species_key: 'taimen', credits_won: '1000000' };
    const fixture = installFishingFixture({
      initialState: {
        pending_round: { round_id: oldRoundId, bait: 'worm', price: '2500000', created_at: 1, auto_settle_at: 2 },
        unrevealed_result: null,
        has_more_unrevealed: false,
      },
      settleFailures: 1,
    ...
    fixture.setState({ pending_round: newerPending, unrevealed_result: oldResult, has_more_unrevealed: false });
    await act(async () => {
      await rendered.queryClient.refetchQueries({ queryKey: fishingKeys.state, exact: true });
    });
    await waitFor(() => expect(document.querySelector('.fishing-stage')?.getAttribute('data-phase')).toBe('casting'));
    await waitFor(() => expect(document.querySelector('.fishing-result')?.getAttribute('data-round-id')).toBe(oldRoundId));
    expect(screen.queryByRole('button', { name: 'Retry settlement' })).not.toBeInTheDocument();

    await waitFor(() => expect(fixture.calls.settle).toBe(2), { timeout: 7_000 });
    expect(document.querySelector('.fishing-result')?.getAttribute('data-round-id')).toBe(oldRoundId);
  }, 15_000);
  ...
  it('drops a delayed start success after a newer recovered result takes authority', async () => {
    ...
    fixture.setState({ pending_round: null, unrevealed_result: newerResult, has_more_unrevealed: false });
    await act(async () => {
      await rendered.queryClient.refetchQueries({ queryKey: fishingKeys.state, exact: true });
    });
    await waitFor(() => expect(document.querySelector('.fishing-result')?.getAttribute('data-round-id')).toBe(newerRoundId));

    releaseStart();
    await waitFor(() => expect(fixture.calls.games).toBeGreaterThanOrEqual(2));
    expect(document.querySelector('.fishing-result')?.getAttribute('data-round-id')).toBe(newerRoundId);
    expect(screen.queryByText('No paid round was created. Choose bait to try again.')).not.toBeInTheDocument();
  }, 10_000);
  ...
  it('drops a delayed start error after a newer pending round takes authority', async () => {
    ...
    fixture.setState({ pending_round: newerPending, unrevealed_result: null, has_more_unrevealed: false });
    await act(async () => {
      await rendered.queryClient.refetchQueries({ queryKey: fishingKeys.state, exact: true });
    });
    await waitFor(() => expect(document.querySelector('.fishing-stage')?.getAttribute('data-phase')).toBe('casting'));
  ...
  it('closes start controls during a state refetch and reopens only after success', async () => {
    let releaseState!: () => void;
    const stateRefetchGate = new Promise<void>((resolve) => { releaseState = resolve; });
  ...
  it('keeps a newer ACK write uncertain when an older authority read finishes first', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('alert')).toHaveTextContent('Temporary acknowledgement outage.'));
    ...
    releaseAckRead();
    await waitFor(() => expect(screen.getByRole('button', { name: 'Mark as viewed' })).toBeEnabled());
    expect(fixture.calls.ack).toBe(1);
  }, 10_000);
  ...
  it('keeps the oldest result visible while a new round settles and queues each ACK', async () => {
    const firstResult = { ...result, round_id: roundId('b'), species_key: 'taimen', credits_won: '1000000', credits: '6000000' };
    const nextResult = { ...result, round_id: roundId('c'), species_key: 'yellowcheek', credits_won: '2000000', credits: '7000000' };
    const { calls } = installFishingFixture({
      initialState: {
        pending_round: { round_id: roundId('b'), bait: 'worm', price: '2500000', created_at: 1, auto_settle_at: 2 },
        unrevealed_result: result,
        has_more_unrevealed: false,
      },
      settleResults: [firstResult, nextResult],
      startRoundId: roundId('c'),
      ackFollowupState: { pending_round: null, unrevealed_result: firstResult, has_more_unrevealed: false },
    });
    const rendered = await renderWithProviders(<FishingGame />, { station: 'user', role: 'user' });
  ...
  it('does not let an ACK started from an older render reset a newer round', async () => {
    ...
    const nextResult = { ...result, round_id: nextRoundId, species_key: 'taimen', credits_won: '1000000', credits: '3500000' };
    const { calls } = installFishingFixture({
      initialState: { pending_round: null, unrevealed_result: result, has_more_unrevealed: false },
      ...
      ackDelayMs: 100,
      ackFollowupState: {
        pending_round: { round_id: nextRoundId, bait: 'worm', price: '2500000', created_at: 1, auto_settle_at: 2 },
        unrevealed_result: null,
        has_more_unrevealed: false,
      },
    });
    ...
    await waitFor(() => expect(calls.settle).toBe(1), { timeout: 7_000 });
    expect(await screen.findByRole('heading', { name: 'Catch confirmed' })).toBeInTheDocument();
    expect(document.querySelector('.fishing-result')?.getAttribute('data-round-id')).toBe(nextRoundId);
  }, 10_000);
  ...
  it('renders a Top20-external mine row and applies privacy changes before refetch', async () => {
    ...
    expect(screen.getByText('Privacy status is being rechecked with the server…')).toBeInTheDocument();

    await waitFor(() => expect(toggle).toBeEnabled(), { timeout: 2_000 });
    await view.user.click(toggle);
    expect(screen.queryByText('Mine only')).not.toBeInTheDocument();
    await waitFor(() => expect(fixture.calls.profile).toBe(2), { timeout: 2_000 });
    await waitFor(() => expect(screen.getByText('Mine only')).toBeInTheDocument(), { timeout: 2_000 });
  ...
  it.each(['malformed', 'no-content', 'wrong'] as const)('reconciles a %s privacy response with the authoritative public state', async (profileResponseMode) => {
    installFishingFixture({ profileResponseMode, leaderboardMe: {
      rank: 21,
      species_key: 'koi',
      size_cm: 180,
      display_name: 'Mine only',
    ...
    await view.user.click(toggle);
    await waitFor(() => expect(toggle).toBeChecked());
    await expect(screen.findByRole('alert')).resolves.toHaveTextContent('invalid fishing profile response');
    expect(await screen.findByText('Mine only')).toBeInTheDocument();
  });

  it('re-reads an accepted privacy state after remount instead of trusting local checkbox state', async () => {
    const fixture = installFishingFixture({ profileResponseMode: 'no-content', leaderboardMe: {
      rank: 21,
      species_key: 'koi',
      size_cm: 180,
      display_name: 'Mine only',
  ...
  it.each([
  ...
  ] as const)('keeps privacy fail-closed and retries all authority reads after a %s revalidation failure', async (_label, reconcileFailures) => {
    const fixture = installFishingFixture({
      reconcileFailures,
      leaderboardMe: {
        rank: 21,
    ...
    const toggle = await screen.findByRole('checkbox', { name: 'Show my profile' });
    await view.user.click(toggle);
    await waitFor(() => expect(screen.getByRole('button', { name: 'Retry privacy check' })).toBeEnabled());
    expect(toggle).not.toBeChecked();
    expect(screen.queryByText('Mine only')).not.toBeInTheDocument();
  ...
  it('masks cached public identity when a background authority read fails and offers retry', async () => {
    ...
    expect(screen.getByRole('checkbox', { name: 'Show my profile' })).not.toBeChecked();
    expect(screen.getByText('Privacy status could not be confirmed. Your identity is hidden until you retry.')).toBeInTheDocument();
  ...
  it('marks the balance unknown after a failed games authority refetch and recovers without a false disabled-game state', async () => {
    ...
    fixture.setReconciliationStarted();
    await view.queryClient.refetchQueries({ queryKey: fishingKeys.config, exact: true });
    await waitFor(() => expect(screen.getByText('The current balance could not be confirmed. No new round can start.')).toBeInTheDocument());
    expect(screen.getByRole('button', { name: 'Cast line' })).toBeDisabled();
    expect(screen.queryByRole('heading', { name: 'Fishing is not open' })).not.toBeInTheDocument();
    expect(screen.getByRole('button', { name: /Worm.*Balance unavailable until the server confirms it/ })).toBeDisabled();
  ...
  it('immediately hides the caller when a later authoritative read changes public to private', async () => {
    ...
    await view.queryClient.refetchQueries({ queryKey: fishingKeys.config, exact: true });
    await waitFor(() => expect(screen.getByRole('checkbox', { name: 'Show my profile' })).not.toBeChecked());
    expect(screen.queryByText('Mine only')).not.toBeInTheDocument();
    expect(screen.getAllByText('Anonymous angler').length).toBeGreaterThan(0);
  });

  it('rejects a settle response for another round without rendering or ACKing it', async () => {
    const { calls } = installFishingFixture({ settleResult: { ...result, round_id: roundId('d') }, settleCredits: '2500000' });
    const rendered = await renderWithProviders(<FishingGame />, { station: 'user', role: 'user' });
    await rendered.user.click(await screen.findByRole('button', { name: 'Cast line' }));
    await waitFor(() => expect(rendered.queryClient.getQueryData<{ credits: string }>(fishingKeys.config)?.credits).toBe('2500000'));
    await screen.findByRole('button', { name: 'Retry settlement' }, { timeout: 7_000 });
    expect(screen.queryByRole('heading', { name: 'Catch confirmed' })).not.toBeInTheDocument();
    expect(calls.ack).toBe(0);
    expect(rendered.queryClient.getQueryData<{ credits: string }>(fishingKeys.config)?.credits).toBe('2500000');
  }, 12_000);
  ...
  it('renders the balance from the refreshed games authority, not the settle payload snapshot', async () => {
    ...
    const rendered = await renderWithProviders(<FishingGame />, { station: 'user', role: 'user' });
    await rendered.user.click(await screen.findByRole('button', { name: 'Cast line' }));
    await waitFor(() => expect(screen.getByTestId('fishing-result-balance')).toHaveTextContent('2,500'), { timeout: 7_000 });
  ...
  ] as const)('keeps a %s-mismatched settlement out of the current pending flow', async (_label, contextMismatch) => {
  ...
  it('does not offer settlement retry when start fails without a server round', async () => {
    const { calls } = installFishingFixture({ startFailures: 1, startFailureCreatesPending: false });
    const rendered = await renderWithProviders(<FishingGame />, { station: 'user', role: 'user' });
    await rendered.user.click(await screen.findByRole('button', { name: 'Cast line' }));
    await waitFor(() => expect(calls.start).toBe(1));
