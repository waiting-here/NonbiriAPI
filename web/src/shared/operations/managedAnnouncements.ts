import { managementRoot, type ManagementRole } from './managedUsers';
...
export const managedAnnouncementKeys = {
  pages: (role: ManagementRole) => [...managementRoot(role), 'announcements-page'] as const,
  page: (
    role: ManagementRole,
    account: string,
    state: string,
    ...
    [...managementRoot(role), 'announcements-page', account, state, severity, page, size] as const,
  detail: (role: ManagementRole, account: string, id: string) =>
    [...managementRoot(role), 'announcement', account, id] as const,
};
const announcementBase = (role: ManagementRole) =>
  role === 'admin' ? '/admin/api/announcements' : '/api/steward/announcements';
...
export async function getAdminAnnouncementsPage(
  ...
  const result = await decoded(
    queryPath(announcementBase(role), {
      state: state || undefined,
      severity: severity || undefined,
      page: pageNumber,
      page_size: pageSize,
...
export const getAdminAnnouncement = (id: string, role: ManagementRole = 'admin') =>
  decoded(
    `${announcementBase(role)}/${encodeURIComponent(opaqueID(id, 'ann_', 'announcement id'))}`,
    normalizeAdminAnnouncement,
  );
export const createAnnouncement = (body: unknown, key: string, role: ManagementRole = 'admin') =>
  decoded(
    announcementBase(role),
...
export const previewAnnouncement = (id: string, body: unknown, role: ManagementRole = 'admin') =>
  decoded(
    `${announcementBase(role)}/${encodeURIComponent(opaqueID(id, 'ann_', 'announcement id'))}/preview`,
...
export async function publishAnnouncement(
  ...
  await apiFetch<void>(
    `${announcementBase(role)}/${encodeURIComponent(opaqueID(id, 'ann_', 'announcement id'))}/publish`,
...
export async function withdrawAnnouncement(
  ...
  await apiFetch<void>(
    `${announcementBase(role)}/${encodeURIComponent(opaqueID(id, 'ann_', 'announcement id'))}/withdraw`,
