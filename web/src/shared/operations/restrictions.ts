...
export function automaticRestrictions(value: unknown) {
  const items = array(value, 'automatic restrictions', 2).map((value) => {
    const item = record(
      value,
      ['kind', 'reason_code', 'reason', 'started_at', 'ends_at'],
  ...
  if (new Set(items.map((item) => item.kind)).size !== items.length)
    invalidResponse('duplicate restriction');
  return items;
}

export type AutomaticRestriction = ReturnType<typeof automaticRestrictions>[number];

export function readLoginRestrictions(fragment: string): AutomaticRestriction[] {
  if (!/^#restrictions=[A-Za-z0-9_-]{1,6000}$/.test(fragment)) return [];
  try {
    const raw = atob(
      fragment.slice('#restrictions='.length).replaceAll('-', '+').replaceAll('_', '/'),
    );
    const text = new TextDecoder('utf-8', { fatal: true }).decode(
      Uint8Array.from(raw, (c) => c.charCodeAt(0)),
    );
    return automaticRestrictions(JSON.parse(text));
  } catch {
    return [];
  }
}
