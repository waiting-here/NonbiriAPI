...
export interface StewardLogDetail {
...
export function normalizeLogUsage(value: unknown): LogUsage {
  ...
  const sum =
    BigInt(result.uncached_input_tokens) +
    BigInt(result.cache_write_input_tokens) +
    BigInt(result.cache_read_input_tokens) +
    BigInt(result.output_tokens);
...
function commonRow(root: WireRecord): LogRowCommon {
  ...
  if (resultClass === null) {
    if (callerStatus !== null || callerErrorCode !== null || completedAt !== null)
      invalidResponse('nonterminal log result');
    ...
    if (
      ...
      callerStatus < 200 ||
      callerStatus > 399 ||
      callerErrorCode !== null
    )
      invalidResponse('successful log result');
    ...
    if (
      ...
      invalidResponse('failed log result');
...
export function normalizeUserLogRow(value: unknown): UserLogRow {
  const probe = record(value, [...COMMON_ROW_FIELDS, 'model', 'attempt_count'], 'user log row', [
    ...COMMON_ROW_FIELDS,
    'model',
  ]);
...
function commonAttempt(root: WireRecord): LogAttemptCommon {
  ...
  try {
    ...
    return invalidResponse('endpoint base URL');
  }
  if (
  ...
  const resultKind = oneOf(
    root.result_kind,
    ['response', 'synthetic'] as const,
    'attempt result kind',
  );
...
export function normalizeUserLogDetail(value: unknown): UserLogDetail {
  const probe = record(value, ['request', 'attempts', 'caller_safe_result'], 'user log detail', [
    'request',
  ]);
  ...
  if (request.kind === 'charity') {
    ...
    return {
      kind: 'charity',
      request,
      caller_safe_result: { class: oneOf(safe.class, RESULT_CLASSES, 'caller-safe class') },
    };
  }
  ...
  return {
    kind: 'self',
    request,
    attempts: page(root.attempts, 'self log attempts', normalizeUserLogAttempt),
  };
}
...
export function normalizeAdminLogDetail(value: unknown): AdminLogDetail {
  ...
  return {
    ...
    attempts: page(root.attempts, 'administrator log attempts', normalizeAdminLogAttempt),
  };
}
...
export function normalizeStewardLogDetail(value: unknown): StewardLogDetail {
  ...
  return {
    ...
    attempts: page(root.attempts, 'steward log attempts', normalizeStewardLogAttempt),
  };
}
...
export function useRoleLogs(
  ...
  return useQuery({
    ...
    queryFn: () =>
      decoded(queryPath(rolePath(role), { ...filter, cursor, limit }), (value) =>
        page(value, `${role} logs`, roleDecoder(role)),
      ),
    enabled,
  });
}
