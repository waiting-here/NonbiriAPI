...
> **当前源码：** 1.0.0-beta.4 开发候选，尚未正式发布。最新已发布版本为 [1.0.0-beta.3](https://github.com/waiting-here/NonbiriAPI/releases/tag/v1.0.0-beta.3)。向用户开放前，请阅读部署、隐私和安全文档。
>
> **兼容边界：** 继续采用 Generation 2（`application_id=0x4E425249`、`user_version=2`），生产目标为 Linux/amd64。支持完整 beta.3 和此前十种精确结构的原子升级，保留现有数据和配置，新增游戏积分钱包为零。Alpha/Generation 1 仍须显式全新切换；具体来源见[部署指南](docs/deployment.md#database-compatibility-and-version-changes)。
>
...
Beta.4 使用 102 张表的 Generation 2 结构。升级先验证精确来源，再原子扩展并检查结构、外键和两种资产的账务。历史通用积分、已结算费用、已有游戏规则、运营配置和自定义法律正文保持；旧局继续按版本 1 排空，新局使用版本 2。不能只换旧二进制降级，须恢复相匹配的完整停服快照。全新数据库仍默认维护开启，注册、活动、公益、捐赠入口和游戏关闭。
