...
>
> **兼容边界：** beta.2 继续采用数据库 Generation 2（`application_id=0x4E425249`、`user_version=2`），以 Linux/amd64 源码构建为发布边界。完全不存在的数据库文件集合可以全新创建；alpha 和 Generation 1 部署必须显式全新切换。四个精确的旧 Generation 2 manifest 与四个精确的已部署 beta.2 中间结构可以在增量更新中保留现有数据，具体来源和更新规则见[部署指南](docs/deployment.md#database-compatibility-and-version-changes)。
>
