...
《竞标对决》提供 13 轮同步暗牌竞标；《回合制对战小游戏（测试）》包含五位角色、八种 Harness 和完整技能／Buff 规则，双方同步播放 5 秒结算，轮初补充与资源变化都有动态反馈。API 余量和金币采用数字展示；未眩晕必须选招，眩晕未解除时才可跳过。角色、技能和 Harness 的专属插画随程序打包。两款游戏支持原币种退票、近 30 天本人历史、管理员匿名长期存档及可继续的分页导出。详见[双语游戏指南](docs/duel-games.md)。
